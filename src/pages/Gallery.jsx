...
// }
import { useEffect, useState, useMemo } from "react";
import { fetchBranches, fetchGalleryImages } from "@/lib/api";
import SEO from "@/components/SEO";
import BreadcrumbSchema from "@/components/seo/BreadcrumbSchema";
...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    // This call was missing entirely — that's why your 2 uploaded photos
    // never appeared, even though the admin upload itself worked fine.
    fetchGalleryImages().then(setStandaloneImages).catch(() => {});
  }, []);
  ...
  const allImages = useMemo(() => {
    ...
    standaloneImages.forEach((img) =>
      all.push({
        src: img.url,
        category: img.category || "Game Zone",
        city: img.branch_slug
          ? branches.find((b) => b.slug === img.branch_slug)?.city || ""
          : "",
      }),
    );

    return all;
  }, [branches, standaloneImages]);
            ...
            {images.map((img, i) => (
              ...
              >
                <div className="relative aspect-[4/3] overflow-hidden">
                  <img
                    src={img.src}
                    alt={img.city || "Game On India"}
                    className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition duration-700"
                  />
                </div>
                <div className="p-3 flex justify-between text-xs">
                  <span className="text-white/60">{img.city || "Game On"}</span>
                  <span className="text-brand-cyan">Game On</span>
                </div>
              </div>
            ))}
